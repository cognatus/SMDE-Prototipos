-- MySQL Script generated by MySQL Workbench
-- 03/17/16 13:22:15
-- Model: New Model    Version: 1.0
-- MySQL Workbench Forward Engineering

SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0;
SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0;
SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='TRADITIONAL,ALLOW_INVALID_DATES';

-- -----------------------------------------------------
-- Schema smdedbv1
-- -----------------------------------------------------

-- -----------------------------------------------------
-- Schema smdedbv1
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `smdedbv1` DEFAULT CHARACTER SET utf8 ;
USE `smdedbv1` ;

-- -----------------------------------------------------
-- Table `smdedbv1`.`Institute`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smdedbv1`.`Institute` (
  `idInstitute` VARCHAR(30) NOT NULL,
  `instituteName` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`idInstitute`),
  UNIQUE INDEX `idinstitute_UNIQUE` (`idInstitute` ASC))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `smdedbv1`.`User`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smdedbv1`.`User` (
  `idUser` VARCHAR(30) NOT NULL,
  `userEmail` VARCHAR(100) NOT NULL,
  `userName` VARCHAR(80) NOT NULL,
  `userLastName` VARCHAR(80) NOT NULL,
  `userSecondLastName` VARCHAR(80) NOT NULL,
  `userSex` CHAR(1) NOT NULL,
  `userAge` TINYINT(3) NULL,
  `Institute_idInstitute` VARCHAR(30) NOT NULL,
  PRIMARY KEY (`userEmail`, `idUser`),
  INDEX `fk_User_Institute1_idx` (`Institute_idInstitute` ASC),
  CONSTRAINT `fk_User_Institute1`
    FOREIGN KEY (`Institute_idInstitute`)
    REFERENCES `smdedbv1`.`Institute` (`idInstitute`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `smdedbv1`.`Department`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smdedbv1`.`Department` (
  `idDepartment` VARCHAR(20) NOT NULL,
  `departmentName` VARCHAR(45) NULL,
  `Institute_idInstitute` VARCHAR(30) NOT NULL,
  PRIMARY KEY (`idDepartment`, `Institute_idInstitute`),
  INDEX `fk_Department_Institute1_idx` (`Institute_idInstitute` ASC),
  CONSTRAINT `fk_Department_Institute1`
    FOREIGN KEY (`Institute_idInstitute`)
    REFERENCES `smdedbv1`.`Institute` (`idInstitute`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `smdedbv1`.`Subject`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smdedbv1`.`Subject` (
  `idSubject` VARCHAR(20) NOT NULL,
  `subjectName` VARCHAR(45) NULL,
  `subjectLevel` VARCHAR(45) NULL,
  `Department_idDepartment` VARCHAR(20) NOT NULL,
  `Department_Institute_idInstitute` VARCHAR(30) NOT NULL,
  PRIMARY KEY (`idSubject`, `Department_idDepartment`, `Department_Institute_idInstitute`),
  INDEX `fk_Subject_Department1_idx` (`Department_idDepartment` ASC, `Department_Institute_idInstitute` ASC),
  CONSTRAINT `fk_Subject_Department1`
    FOREIGN KEY (`Department_idDepartment` , `Department_Institute_idInstitute`)
    REFERENCES `smdedbv1`.`Department` (`idDepartment` , `Institute_idInstitute`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `smdedbv1`.`Group`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smdedbv1`.`Group` (
  `idGroup` VARCHAR(10) NOT NULL,
  `groupName` VARCHAR(10) NOT NULL,
  `groupLevel` INT NOT NULL,
  PRIMARY KEY (`idGroup`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `smdedbv1`.`User`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smdedbv1`.`User` (
  `idUser` VARCHAR(30) NOT NULL,
  `userEmail` VARCHAR(100) NOT NULL,
  `userName` VARCHAR(80) NOT NULL,
  `userLastName` VARCHAR(80) NOT NULL,
  `userSecondLastName` VARCHAR(80) NOT NULL,
  `userSex` CHAR(1) NOT NULL,
  `userAge` TINYINT(3) NULL,
  `Institute_idInstitute` VARCHAR(30) NOT NULL,
  PRIMARY KEY (`userEmail`, `idUser`),
  INDEX `fk_User_Institute1_idx` (`Institute_idInstitute` ASC),
  CONSTRAINT `fk_User_Institute1`
    FOREIGN KEY (`Institute_idInstitute`)
    REFERENCES `smdedbv1`.`Institute` (`idInstitute`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `smdedbv1`.`Menssage`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smdedbv1`.`Menssage` (
  `idMessage` INT NOT NULL,
  `messageText` MEDIUMTEXT NULL,
  `messageDateTime` DATETIME NOT NULL,
  `messageStatus` TINYINT(1) NOT NULL,
  `messageReceptor` VARCHAR(20) NOT NULL,
  `User_userEmail` VARCHAR(100) NOT NULL,
  `messageToUser` VARCHAR(100) NULL,
  PRIMARY KEY (`idMessage`, `User_userEmail`),
  UNIQUE INDEX `idMessage_UNIQUE` (`idMessage` ASC),
  INDEX `fk_Menssage_User1_idx` (`User_userEmail` ASC),
  CONSTRAINT `fk_Menssage_User1`
    FOREIGN KEY (`User_userEmail`)
    REFERENCES `smdedbv1`.`User` (`userEmail`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `smdedbv1`.`attend`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smdedbv1`.`attend` (
  `Student_idStudent` VARCHAR(30) NOT NULL,
  PRIMARY KEY (`Student_idStudent`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `smdedbv1`.`Teacher`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smdedbv1`.`Teacher` (
  `idTeacher` INT NOT NULL,
  PRIMARY KEY (`idTeacher`),
  CONSTRAINT `fk_Teacher_User1`
    FOREIGN KEY (`idTeacher`)
    REFERENCES `smdedbv1`.`User` (`idUser`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `smdedbv1`.`Notification`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smdedbv1`.`Notification` (
  `idNotification` INT NOT NULL,
  `notifTitle` VARCHAR(60) NULL,
  `notifText` MEDIUMTEXT NULL,
  `notifDate` DATE NULL,
  `notifTime` TIME(1) NULL,
  PRIMARY KEY (`idNotification`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `smdedbv1`.`Publication`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smdedbv1`.`Publication` (
  `idPublication` INT NOT NULL,
  `pubTitle` VARCHAR(100) NULL,
  `pubText` MEDIUMTEXT NULL,
  `pubDateTime` DATETIME NULL,
  `Teacher_idTeacher` INT NOT NULL,
  `Notification_idNotification` INT NOT NULL,
  PRIMARY KEY (`idPublication`, `Teacher_idTeacher`, `Notification_idNotification`),
  INDEX `fk_Publication_Teacher1_idx` (`Teacher_idTeacher` ASC),
  INDEX `fk_Publication_Notification1_idx` (`Notification_idNotification` ASC),
  CONSTRAINT `fk_Publication_Teacher1`
    FOREIGN KEY (`Teacher_idTeacher`)
    REFERENCES `smdedbv1`.`Teacher` (`idTeacher`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Publication_Notification1`
    FOREIGN KEY (`Notification_idNotification`)
    REFERENCES `smdedbv1`.`Notification` (`idNotification`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `smdedbv1`.`Student`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smdedbv1`.`Student` (
  `idStudent` INT NOT NULL,
  PRIMARY KEY (`idStudent`),
  CONSTRAINT `fk_Student_User1`
    FOREIGN KEY (`idStudent`)
    REFERENCES `smdedbv1`.`User` (`idUser`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `smdedbv1`.`Feedback`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smdedbv1`.`Feedback` (
  `idFeedback` INT NOT NULL,
  `feedTitle` VARCHAR(45) NULL,
  `feedText` MEDIUMTEXT NULL,
  `feedDateTime` DATETIME NULL,
  `Student_idStudent` INT NOT NULL,
  `Publication_idPublication` INT NOT NULL,
  `Publication_Teacher_idTeacher` INT NOT NULL,
  `Notification_idNotification` INT NOT NULL,
  PRIMARY KEY (`idFeedback`, `Student_idStudent`, `Publication_idPublication`, `Publication_Teacher_idTeacher`, `Notification_idNotification`),
  INDEX `fk_Feedback_Student1_idx` (`Student_idStudent` ASC),
  INDEX `fk_Feedback_Publication1_idx` (`Publication_idPublication` ASC, `Publication_Teacher_idTeacher` ASC),
  INDEX `fk_Feedback_Notification1_idx` (`Notification_idNotification` ASC),
  CONSTRAINT `fk_Feedback_Student1`
    FOREIGN KEY (`Student_idStudent`)
    REFERENCES `smdedbv1`.`Student` (`idStudent`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Feedback_Publication1`
    FOREIGN KEY (`Publication_idPublication` , `Publication_Teacher_idTeacher`)
    REFERENCES `smdedbv1`.`Publication` (`idPublication` , `Teacher_idTeacher`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Feedback_Notification1`
    FOREIGN KEY (`Notification_idNotification`)
    REFERENCES `smdedbv1`.`Notification` (`idNotification`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `smdedbv1`.`Content`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smdedbv1`.`Content` (
  `idContent` INT NOT NULL,
  `contentTitle` VARCHAR(45) NULL,
  `contentDateTime` DATETIME NULL,
  `contentText` MEDIUMTEXT NULL,
  `Notification_idNotification` INT NOT NULL,
  `Teacher_idTeacher` INT NOT NULL,
  PRIMARY KEY (`idContent`, `Notification_idNotification`, `Teacher_idTeacher`),
  INDEX `fk_Content_Notification1_idx` (`Notification_idNotification` ASC),
  INDEX `fk_Content_Teacher1_idx` (`Teacher_idTeacher` ASC),
  CONSTRAINT `fk_Content_Notification1`
    FOREIGN KEY (`Notification_idNotification`)
    REFERENCES `smdedbv1`.`Notification` (`idNotification`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Content_Teacher1`
    FOREIGN KEY (`Teacher_idTeacher`)
    REFERENCES `smdedbv1`.`Teacher` (`idTeacher`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `smdedbv1`.`ForumTopic`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smdedbv1`.`ForumTopic` (
  `idForumTopic` INT NOT NULL,
  `topicTitle` VARCHAR(100) NOT NULL,
  `topicSubject` VARCHAR(100) NOT NULL,
  PRIMARY KEY (`idForumTopic`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `smdedbv1`.`Note`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smdedbv1`.`Note` (
  `idNote` INT NOT NULL,
  `noteValue` VARCHAR(3) NULL,
  `noteSubject` VARCHAR(45) NULL,
  `Subject_idSubject` INT NOT NULL,
  `Student_idStudent` INT NOT NULL,
  PRIMARY KEY (`idNote`, `Subject_idSubject`, `Student_idStudent`),
  INDEX `fk_Note_Subject1_idx` (`Subject_idSubject` ASC),
  INDEX `fk_Note_Student1_idx` (`Student_idStudent` ASC),
  CONSTRAINT `fk_Note_Subject1`
    FOREIGN KEY (`Subject_idSubject`)
    REFERENCES `smdedbv1`.`Subject` (`idSubject`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Note_Student1`
    FOREIGN KEY (`Student_idStudent`)
    REFERENCES `smdedbv1`.`Student` (`idStudent`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `smdedbv1`.`Reminder`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smdedbv1`.`Reminder` (
  `idReminder` INT NOT NULL,
  `reminderText` MEDIUMTEXT NULL,
  `reminderDateTime` DATETIME NOT NULL,
  `User_userEmail` VARCHAR(100) NOT NULL,
  `User_idUser` VARCHAR(30) NOT NULL,
  PRIMARY KEY (`idReminder`, `User_userEmail`, `User_idUser`),
  INDEX `fk_Reminder_User1_idx` (`User_userEmail` ASC, `User_idUser` ASC),
  CONSTRAINT `fk_Reminder_User1`
    FOREIGN KEY (`User_userEmail` , `User_idUser`)
    REFERENCES `smdedbv1`.`User` (`userEmail` , `idUser`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `smdedbv1`.`Group_has_Subject`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smdedbv1`.`Group_has_Subject` (
  `Group_idGroup` VARCHAR(10) NOT NULL,
  `Subject_idSubject` VARCHAR(20) NOT NULL,
  PRIMARY KEY (`Group_idGroup`, `Subject_idSubject`),
  INDEX `fk_Group_has_Subject_Subject1_idx` (`Subject_idSubject` ASC),
  INDEX `fk_Group_has_Subject_Group1_idx` (`Group_idGroup` ASC),
  CONSTRAINT `fk_Group_has_Subject_Group1`
    FOREIGN KEY (`Group_idGroup`)
    REFERENCES `smdedbv1`.`Group` (`idGroup`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Group_has_Subject_Subject1`
    FOREIGN KEY (`Subject_idSubject`)
    REFERENCES `smdedbv1`.`Subject` (`idSubject`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `smdedbv1`.`ForumComent`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smdedbv1`.`ForumComent` (
  `idForumComent` INT NOT NULL,
  `forumText` VARCHAR(45) NULL,
  `ForumTopic_idForumTopic` INT NOT NULL,
  `User_userEmail` VARCHAR(100) NOT NULL,
  PRIMARY KEY (`idForumComent`, `ForumTopic_idForumTopic`, `User_userEmail`),
  INDEX `fk_ForumComent_ForumTopic1_idx` (`ForumTopic_idForumTopic` ASC),
  INDEX `fk_ForumComent_User1_idx` (`User_userEmail` ASC),
  CONSTRAINT `fk_ForumComent_ForumTopic1`
    FOREIGN KEY (`ForumTopic_idForumTopic`)
    REFERENCES `smdedbv1`.`ForumTopic` (`idForumTopic`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_ForumComent_User1`
    FOREIGN KEY (`User_userEmail`)
    REFERENCES `smdedbv1`.`User` (`userEmail`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `smdedbv1`.`Teacher_has_Group`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smdedbv1`.`Teacher_has_Group` (
  `Teacher_idTeacher` INT NOT NULL,
  `Group_idGroup` VARCHAR(10) NOT NULL,
  PRIMARY KEY (`Teacher_idTeacher`, `Group_idGroup`),
  INDEX `fk_Teacher_has_Group_Group1_idx` (`Group_idGroup` ASC),
  INDEX `fk_Teacher_has_Group_Teacher1_idx` (`Teacher_idTeacher` ASC),
  CONSTRAINT `fk_Teacher_has_Group_Teacher1`
    FOREIGN KEY (`Teacher_idTeacher`)
    REFERENCES `smdedbv1`.`Teacher` (`idTeacher`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Teacher_has_Group_Group1`
    FOREIGN KEY (`Group_idGroup`)
    REFERENCES `smdedbv1`.`Group` (`idGroup`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `smdedbv1`.`Student_has_Group`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smdedbv1`.`Student_has_Group` (
  `Student_idStudent` INT NOT NULL,
  `Group_idGroup` VARCHAR(10) NOT NULL,
  PRIMARY KEY (`Student_idStudent`, `Group_idGroup`),
  INDEX `fk_Student_has_Group_Group1_idx` (`Group_idGroup` ASC),
  INDEX `fk_Student_has_Group_Student1_idx` (`Student_idStudent` ASC),
  CONSTRAINT `fk_Student_has_Group_Student1`
    FOREIGN KEY (`Student_idStudent`)
    REFERENCES `smdedbv1`.`Student` (`idStudent`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Student_has_Group_Group1`
    FOREIGN KEY (`Group_idGroup`)
    REFERENCES `smdedbv1`.`Group` (`idGroup`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


SET SQL_MODE=@OLD_SQL_MODE;
SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS;
SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS;
